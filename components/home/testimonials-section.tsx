...
export default function TestimonialsSection() {
  const {
    ...
    isLoading,
    error,
  } = useDataFetch<Testimonial[]>(portfolioData.getTestimonials)
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="glass-card p-8 rounded-xl relative animate-pulse"
          >
          ...
          <GlassCardContent className="p-8">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-purple-500 glow">
                <Image
                  src={
                    testimonial.image || '/placeholder.svg?height=80&width=80'
                  }
