...
export default function ProblemSolutionSection() {
  const { data: items, isLoading, error } = useDataFetch<ProblemSolution[]>(portfolioData.getProblemSolutions)

  if (isLoading) {
    return (
      <div className="space-y-8">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="glass-card p-8 rounded-xl relative overflow-hidden group animate-pulse">
          ...
          <GlassCardContent className="p-8">
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <h3 className="text-xl font-semibold group-hover:text-white transition-colors">{item.problem}</h3>
                <p className="text-zinc-400 group-hover:text-zinc-300 transition-colors">{item.description}</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <h3 className="text-xl font-semibold group-hover:text-white transition-colors">{item.solution}</h3>
                <p className="text-zinc-400 group-hover:text-zinc-300 transition-colors">{item.solutionDescription}</p>
              </div>
            </div>
          </GlassCardContent>
        </GlassCard>
